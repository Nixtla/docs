...
```python
tune_to_features(target_features, min_length, max_length, freq, seasonal_period=None, n_generations=15, population_size=30, n_draws_per_candidate=3, tolerance=0.05, seed=None, *, n_jobs=-1, **generator_kwargs)
```
...
#### `MARGenerator.tuning_diagnostics`

```python
tuning_diagnostics: dict[str, float | int | bool] | None
```
