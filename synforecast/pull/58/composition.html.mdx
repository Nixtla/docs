...
```python
balanced_pool(min_length=200, max_length=200, freq='D', seed=42, seasonal_period=None, **base_kwargs)
```
...
```python
pretraining_pool(min_length=256, max_length=1024, freq='D', seed=42, include_balanced=True, n_meta_variants=3, seasonal_period=None, **base_kwargs)
```
...
`seasonal_period` | <code>[int](#int) \| None</code> | Seasonal period for the seasonal variants of the included :func:`balanced_pool`. Defaults to None, which derives it from ``freq``. | <code>None</code>
`**base_kwargs` | <code>[Any](#typing.Any)</code> | Additional keyword arguments passed to all generators (e.g., engine, id_col, time_col, target_col). | <code>&#123;&#125;</code>
