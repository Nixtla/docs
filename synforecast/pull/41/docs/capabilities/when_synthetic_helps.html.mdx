...
>     and the [balanced
>     pool](../../synforecast/docs/capabilities/balanced_pool.html) span
>     behaviors — volatility clustering, long memory, intermittency,
>     chaos — that no single real dataset contains. Useful for
>     stress-testing a pipeline and for pretraining breadth.
> -   **Robustness testing.** Inject
>     [anomalies](../../synforecast/docs/capabilities/anomalies.html),
>     [changepoints](../../synforecast/docs/capabilities/changepoints.html),
>     and
>     [missingness](../../synforecast/docs/capabilities/missingness.html)
>     with known ground truth to measure how a model degrades.
...
>     [TCM](../../synforecast/docs/generators/pretraining/tcm.html), and
>     [KernelSynth](../../synforecast/docs/generators/pretraining/kernel_synth.html)
>     — exist to pretrain models that transfer to unseen series. The
>     section below measures this: a synthetic-pretrained model used
>     zero-shot beats training from scratch when the real history is
>     short.
...
corpus from
[`pretraining_pool()`](../../synforecast/docs/capabilities/balanced_pool.html),
then forecast M3 Monthly series while varying how much history each
series keeps, from 48 months up to the full record. The corpus is
generated independently and never sees M3.
