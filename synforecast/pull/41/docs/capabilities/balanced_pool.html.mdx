...
and a bias-free starting point for benchmarking or pretraining:
generators are allocated proportionally to each niche’s behavioral
range, so no single domain dominates the pool. The list is ordered
...
[when synthetic data
helps](../../synforecast/docs/capabilities/when_synthetic_helps.html)
for a paired, multi-seed benchmark that reports where it wins, where it
is neutral, and where the edge reverses.
