...
`generators` | <code>[list](#list)[[BaseGenerator](#synforecast.base.BaseGenerator)]</code> | Generators to draw from. Defaults to `synforecast.interpretable_pool`. Ignores min_length / max_length / freq / engine / seed when provided. | <code>None</code>
...
```python
interpretable_pool(min_length=200, max_length=200, freq='D', seed=42, seasonal_period=None, **base_kwargs)
```
...
`seasonal_period` | <code>[int](#int) \| None</code> | Seasonal period, in time steps, for the seasonal variants. Defaults to None, which derives it from ``freq``. | <code>None</code>
...
```python
pretraining_pool(min_length=256, max_length=1024, freq='D', seed=42, include_balanced=True, n_meta_variants=3, seasonal_period=None, include_seasonal=False, **base_kwargs)
```
...
`include_balanced` | <code>[bool](#bool)</code> | When True (default), prepend the full :func:`interpretable_pool`; when False, return only the meta-generators (a purely procedural pretraining corpus), plus the seasonal instances if ``include_seasonal`` is also set. | <code>True</code>
...
`seasonal_period` | <code>[int](#int) \| None</code> | Seasonal period for the seasonal variants of the included :func:`interpretable_pool` and :func:`seasonal_pool`. Defaults to None, which derives it from ``freq``. | <code>None</code>
`include_seasonal` | <code>[bool](#bool)</code> | When True, append the eight :func:`seasonal_pool` instances (strong seasonality on a moving level) at the same period. Skipped with a warning when the period is below 2, as for yearly data. Recommended for regularly seasonal quarterly and monthly targets, where it raised feature-space coverage on every panel tested that had a seasonal gap to fill; it does not help intermittent monthly data, where displacing pool breadth cost about three points. Leave it off for weekly, daily, hourly, and intermittent targets. Default False. | <code>False</code>
`**base_kwargs` | <code>[Any](#typing.Any)</code> | Additional keyword arguments passed to all generators (e.g., engine, id_col, time_col, target_col). | <code>&#123;&#125;</code>
...
```python
seasonal_pool(min_length=200, max_length=200, freq='MS', seed=42, seasonal_period=None, **base_kwargs)
```
...
Name | Type | Description | Default
---- | ---- | ----------- | -------
`min_length` | <code>[int](#int)</code> | Minimum series length for all generators. | <code>200</code>
`max_length` | <code>[int](#int)</code> | Maximum series length for all generators. | <code>200</code>
`freq` | <code>[str](#str) \| [int](#int)</code> | Frequency for all generators, as a pandas offset alias or integer. Defaults to ``"MS"``, the frequency this pool was evaluated at, which differs from the ``"D"`` default of :func:`interpretable_pool` and :func:`pretraining_pool`. Pass the same ``freq`` to both when combining them, or the combined corpus mixes frequencies. | <code>'MS'</code>
`seed` | <code>[int](#int) \| None</code> | Base random seed; generator ``i`` receives ``seed + i``. Set to None for random seeds. | <code>42</code>
`seasonal_period` | <code>[int](#int) \| None</code> | Seasonal period in time steps. Defaults to None, which derives it from ``freq`` as in :func:`interpretable_pool`; it must be at least 2, so yearly frequencies need an explicit value. | <code>None</code>
`**base_kwargs` | <code>[Any](#typing.Any)</code> | Additional keyword arguments passed to all generators (e.g., engine, id_col, time_col, target_col). | <code>&#123;&#125;</code>
...
Type | Description
---- | -----------
<code>[list](#list)[[BaseGenerator](#synforecast.base.BaseGenerator)]</code> | List of 8 BaseGenerator instances ready for use with SynSet.
