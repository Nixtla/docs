...
>     from — see [SynAugment](../capabilities/augmentation.html).
...
`generate_series` draws from the interpretable pool of generators
spanning trends, seasonality, volatility clustering, counts, and more,
then returns a panel you can hand straight to any Nixtla forecaster.
...
> the capability guides: [anomalies](../capabilities/anomalies.html),
> [changepoints](../capabilities/changepoints.html), and
> [missingness](../capabilities/missingness.html). To attach exogenous
> regressors, see [exogenous](../capabilities/exogenous.html).
...
-   **Generators** — 32 generator classes from SARIMA, ETS, and GARCH to
    Gaussian processes and the
    [KernelSynth](../generators/pretraining/kernel_synth.html) and
    [MAR](../generators/pretraining/mar.html) pretraining recipes. Start
    with [random walk](../generators/statistical/random_walk.html) or
    [seasonal](../generators/statistical/seasonal.html).
-   **[Augmentation](../capabilities/augmentation.html)** — expand a
    small real panel with `SynAugment` or TSMixup.
-   **[Interpretable pool](../capabilities/balanced_pool.html)** — what
    `generate_series` draws from by default, and how to tailor it.
-   **Integrations** — end-to-end workflows with
    [statsforecast](../integrations/statsforecast.html),
    [mlforecast](../integrations/mlforecast.html), and
    [neuralforecast](../integrations/neuralforecast.html).
