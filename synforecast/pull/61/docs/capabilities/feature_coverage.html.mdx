...
pools fall short. Every number on this page is read from a benchmark
summary rather than transcribed by hand; the scripts that produced them
...
>     52–88% of fresh held-out real queries at the real-calibrated
...
number of median crossings. The definitions are frozen as `native_v1`;
the seasonal period and window width are extraction parameters that you
...
-   **Yearly and Quarterly are essentially covered.** The pools sit
    within a few points of what real candidates achieve. Monthly falls
    about 5 points short and Weekly about 20, the latter flagged as a
    minor gap. Daily is the widest of the three, roughly 25 to 30 points
    below the real-to-real baseline, and depends strongly on which real
    series are queried (two folds gap by more than 25 points, two by
    less than 10).
-   **The two pools are interchangeable here.** They stay within five
    ...
    Random MAR is far behind on every panel, Quarterly included, where
    it reaches 34% against 88% for the pools.
...
against adding random MAR shows the preset helps where seasonality is
strong and is within a point either way elsewhere, including −0.8 points
on car parts. That is how the preset is documented: an opt-in addition
on top of a pool, kept out of the defaults.
...
quarterly and monthly panel, by 2 to 14 points, and lowered it on
weekly, daily, and intermittent panels, by up to about 6 points on M4
weekly and daily and about 3 on car parts, while traffic weekly and
weather daily were essentially unchanged; it builds a cycle those series
do not have and displaces breadth. Six panels failed the no-harm gate,
so the default stays off, and the flag is recommended for quarterly and
monthly targets only. The table below is from that run.
...
The benchmark scripts and their artifacts are not yet in the repository;
they land in a follow-up pull request that keeps this one focused on the
library change. The commands below are recorded now so the protocol is
on the record, and the numbers on this page come from a local run of
exactly these steps. Once the scripts land they run in sequence, each
reading the previous one’s `summary.json`, with detailed per-query
traces written alongside and a README per directory documenting
...
Summaries are written under `benchmarks/data/feature_coverage_*/`, which
is untracked. The API used above, `compute_features` and
`compare_feature_coverage`, is documented in the [evaluation
reference](../../evaluation.html).
