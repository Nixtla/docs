...
here we load the summary it writes; that artifact is untracked, so this
page publishes stored outputs and re-running it needs a local benchmark
run first.
...
>     [`generate_series`](../getting-started/quickstart.html) and the
>     [balanced pool](./balanced_pool.html) span behaviors — volatility
>     clustering, long memory, intermittency, chaos — that no single
>     real dataset contains. Useful for stress-testing a pipeline and
>     for pretraining breadth; [feature-space
>     coverage](./feature_coverage.html) measures how much of real M4
>     behaviour the pools reach.
> -   **Robustness testing.** Inject [anomalies](./anomalies.html),
>     [changepoints](./changepoints.html), and
>     [missingness](./missingness.html) with known ground truth to
>     measure how a model degrades.
...
>     [TSI](../generators/pretraining/tsi.html),
>     [TCM](../generators/pretraining/tcm.html),
>     [KernelSynth](../generators/pretraining/kernel_synth.html), and
>     [MAR](../generators/pretraining/mar.html) — exist to pretrain
>     models that transfer to unseen series. The section below measures
>     this: a synthetic-pretrained model used zero-shot beats training
>     from scratch when the real history is short.
...
corpus from [`pretraining_pool()`](./balanced_pool.html), then forecast
M3 Monthly series while varying how much history each series keeps, from
48 months up to the full record. The corpus is generated independently
and never sees M3.
