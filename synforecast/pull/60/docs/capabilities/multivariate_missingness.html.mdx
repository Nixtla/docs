...
[missingness](./missingness.html) patterns to multivariate generators
(`VAR`, `CopulaGenerator`), so you can test multivariate imputation and
check that a model still recovers the cross-series structure through the
gaps.
