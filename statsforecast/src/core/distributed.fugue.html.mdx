...
| fallback_model | Optional | Any, optional (default=None)<br/>Model to be used if a model fails.<br/>Only works with the `forecast` and [`cross_validation`](src/mfles.html#cross_validation) methods. |
...
[`FugueBackend`](src/core/distributed.fugue.html#fuguebackend)
