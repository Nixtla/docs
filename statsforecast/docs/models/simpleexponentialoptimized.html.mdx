...
description: Use SimpleExponentialSmoothingOptimized in StatsForecast to fit a per-series alpha smoothing parameter by minimizing squared error on each unique_id.
---
...
`SimpleExponentialSmoothingOptimized` fits `alpha` independently for each
`unique_id` in your DataFrame by minimizing squared error, so every series
gets its own smoothing parameter. There are no arguments to constrain the
`alpha` search range. If you want to compare a small set of fixed alphas
across all series, use
[`SimpleExponentialSmoothing`](/statsforecast/docs/models/simpleexponentialsmoothing.html)
with multiple instances instead.
