---
title: ARIMA Model
description: Step-by-step guide to forecasting with the ARIMA model in StatsForecast, covering p, d, q parameters, seasonal ARIMA, and exogenous variables.
---
...
> **Note**
>
> StatsForecast doesn’t have a separate `SARIMAX` class. The `ARIMA`
> and `AutoARIMA` models already cover SARIMAX functionality: set
> `season_length` for the seasonal component and include exogenous
> variables as extra columns in `df`, passing their future values
> through `X_df`. See the [exogenous variables
> guide](../how-to-guides/exogenous.html) for a complete example.
