...
description: Evaluate model performance with time series cross-validation in StatsForecast, using sliding windows, cutoffs, and RMSE to compare forecasts.
---
...
> **Note**
>
> `h` and `step_size` are measured in timesteps of your data’s
> frequency, not in calendar time. For example, if each row of your
> data is one hour, `step_size = 24` moves each window forward by 24
> hourly observations, regardless of how often new data arrives.
...
`cross_validation` doesn’t accept a list of explicit cutoff dates. The
validation windows are always anchored to the end of `df`:

-   The last cutoff is the last timestamp in `df` minus `h` timesteps.
-   Each earlier cutoff is `step_size` timesteps before the next one,
    for a total of `n_windows` cutoffs.
-   Alternatively, you can set `test_size` (the total length of the
    test period), in which case `n_windows` is computed automatically.

To evaluate a specific cutoff date, trim `df` so that it ends exactly
`h` timesteps after the desired cutoff and run `cross_validation` with
`n_windows = 1`. Repeat this for each cutoff you need and concatenate
the results.
