---
description: >-
  NeuralForecast class for fitting, predicting, and cross-validating PyTorch forecasting models on pandas, polars, or Spark DataFrames of time series.
...
`h` | <code>[int](#int)</code> | Forecasting horizon. If `None`, uses the horizon each model was fitted with. Must be greater than or equal to the fitted horizon; see the note above for the restrictions that apply when `h` exceeds it. | <code>None</code>
...
`level` | <code>list of ints or floats</code> | Confidence levels between 0 and 100. | <code>None</code>
`quantiles` | <code>list of floats</code> | Alternative to level, target quantiles to predict. | <code>None</code>
`h` | <code>[int](#int)</code> | Forecasting horizon. If `None`, uses the horizon each model was fitted with. The same rules as `NeuralForecast.predict` apply: `h` must be greater than or equal to the fitted horizon, and the restrictions on `h > fitted_h` (recursive rollout for direct models, no `prediction_intervals`, no `hist_exog_list`) apply here too. | <code>None</code>
`data_kwargs` | <code>[kwargs](#kwargs)</code> | Extra arguments to be passed to the dataset within each model. | <code>{}</code>
