---
title: Cross-validation for NeuralForecast models
description: Run rolling-origin cross-validation on NeuralForecast models with n_windows, val_size, test_size, step_size, refit, and overlapping windows.
---
...
>
> Every series must have at least `input_size + h` observations before
> the first cutoff. Series that fall short at a given cutoff produce
> fewer than `h` predictions for that window, which shows up as some
> `unique_id`/`ds` combinations having only 1, 2, ... rows instead of the
> full horizon. Either filter out series that are too short, extend the
> history, or pass `start_padding_enabled=True` to the model so short
> series are left-padded with zeros up to `input_size`.
