---
title: Time series scaling with local and temporal scalers
description: Scale time series in NeuralForecast with local_scaler_type and per-model scaler_type, including standard, robust, robust-iqr, minmax, and boxcox options.
---
...
scalers are `minmax`, `robust`, `robust-iqr`, `minmax1`, and `boxcox`.

Both robust scalers center each series by its median. They differ in
how they measure spread:

-   `robust` divides by the **median absolute deviation (MAD)**.
-   `robust-iqr` divides by the **inter-quartile range (IQR)**.

Both are outlier-resistant. See [`LocalRobustScaler`](/coreforecast/scalers)
in `coreforecast` for the underlying implementation.
